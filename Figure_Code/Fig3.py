...
def Coexist(psi0,zeta):
	...
	if np.isnan(spinodalpoints[0]):
		...
		common_t = pso(functiontominimize,[0.91,spinodalpoints[1]],[spinodalpoints[0],1],args=(psi0,zeta))[0]    
...
N=1000
lambdalist=np.linspace(0.9,1.0,num=N)
zetalist=np.linspace(1.01,1.6,num=N)

psi0 = 0.4
''' This calculation takes a while, so I have the data saved in the folder Psi0_Coexistance_Data
Lambda_Low,Lambda_High = CoexistenceRegion(psi0,zetalist)
np.savetxt('LambdaLow_Psi0_04.csv',Lambda_Low,delimiter=',')
np.savetxt('LambdaHigh_Psi0_04.csv',Lambda_High,delimiter=',')
'''
...
ax1.text(-15,1.8,'coexistence',fontsize=16)
ax1.text(-13.7,1.7,'region',fontsize=16)
ax1.text(-2.3,1.6,r'$\varepsilon_L$',fontsize=20)
ax1.text(-15,1.12,r'$\varepsilon_H$',fontsize=20)
ax1.arrow(-2.5,1.6,-1.9,0,head_length=0.3,head_width=0.03,length_includes_head=True)
ax1.arrow(-14.5,1.2,0,0.11,head_length=0.02,head_width=0.3,length_includes_head=True)



ax1.set_title('A)',loc='left',fontsize=20)
ax1.set_ylabel('$\zeta$',fontsize=20,rotation=0,labelpad=15)
ax1.set_xlabel(r'Axial Strain ,\Huge$\varepsilon$',fontsize=16)
...
ax2.set_xticks([-0.3,-0.25,-0.2,-0.15,-0.1,-0.05,0])
