...
psi[0]=np.pi/2
ax2.plot(radius,psi,label=r'$\varepsilon_H = -12.4\%$',lw=3,ls='--',color='xkcd:purple')

strain = Lambda_Low
psi=[]
for j in range(N):
    psi.append(ExactPsi(psi0[j],strain,zeta))
ax2.plot(radius,psi,label=r'$\varepsilon_L = -2.6\%$',lw=2.5,ls='-.',color='xkcd:green')
...
for j in range(N):
    psi.append(ExactPsi(psi0[j],strain,zeta))
ax2.plot(radius,psi,label=r'$\varepsilon = +5\%$',lw=2,ls=':',color='black')
